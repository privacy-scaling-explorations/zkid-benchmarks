...
uint get_total_signal_no() {return 3621985;}

uint get_number_of_components() {return 3338;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 752577;}
...
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[1032]{0};
}
...
uint aux_create = 0;
int aux_cmp_num = 0+ctx_index+1;
uint csoffset = mySignalStart+9760;
...
uint csoffset = mySignalStart+18976;
...
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * ((8 * Fr_toInt(&lvar[2])) + Fr_toInt(&lvar[3]))) + 1056)];
// load src
...
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 1056)];
// load src
// end load src
Fr_copy(aux_dest,&circuitConstants[2]);
}
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 1056)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
cmp_index_ref_load = ((1 * Fr_toInt(&expaux[2])) + 1024);
cmp_index_ref_load = ((1 * Fr_toInt(&expaux[2])) + 1024);
Fr_mul(&expaux[4],&lvar[2],&circuitConstants[66]); // line circom 37
Fr_add(&expaux[3],&expaux[4],&lvar[3]); // line circom 37
Fr_add(&expaux[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[2])) + 1024)]].signalStart + ((1 * Fr_toInt(&lvar[3])) + 0)],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[3])) + 1056)]); // line circom 37
Fr_sub(&expaux[5],&lvar[2],&circuitConstants[2]); // line circom 38
cmp_index_ref_load = ((1 * Fr_toInt(&expaux[5])) + 1024);
cmp_index_ref_load = ((1 * Fr_toInt(&expaux[5])) + 1024);
Fr_mul(&expaux[3],&circuitConstants[4],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[5])) + 1024)]].signalStart + ((1 * Fr_toInt(&lvar[3])) + 0)]); // line circom 38
Fr_mul(&expaux[5],&lvar[2],&circuitConstants[66]); // line circom 38
Fr_add(&expaux[4],&expaux[5],&lvar[3]); // line circom 38
Fr_mul(&expaux[2],&expaux[3],&signalValues[mySignalStart + ((1 * Fr_toInt(&expaux[4])) + 1056)]); // line circom 38
...
cmp_index_ref_load = ((1 * Fr_toInt(&expaux[0])) + 1024);
cmp_index_ref_load = ((1 * Fr_toInt(&expaux[0])) + 1024);
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&expaux[0])) + 1024)]].signalStart + ((1 * Fr_toInt(&lvar[3])) + 0)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
KeccakF1600_4_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_mul(&expaux[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1031]].signalStart + ((1 * ((8 * Fr_toInt(&lvar[2])) + Fr_toInt(&lvar[4]))) + 0)],&expaux[2]); // line circom 47
...
Keccak_256_bytes_5_create(mySignalStart+1088,0+ctx_index+1,ctx,new_cmp_name,myId);
...
{
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 0)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 1056)]); // line circom 15
}}
