...
uint get_total_signal_no() {return 452833;}

uint get_number_of_components() {return 419;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 93185;}

uint get_size_of_constants() {return 67;}
...
KeccakF1600_4_create(mySignalStart+2400,128+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[128] = 128+ctx_index+1;
}
{
PFrElement aux_dest = &lvar[1];
// load src
...
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * ((8 * Fr_toInt(&lvar[2])) + Fr_toInt(&lvar[3]))) + 160)];
// load src
...
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[2])) + 160)];
// load src
// end load src
Fr_copy(aux_dest,&circuitConstants[2]);
}
...
while(Fr_isTrue(&expaux[0])){
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[64]); // line circom 30
if(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[3])) + 160)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[66]); // line circom 45
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[3];
// load src
...
Fr_mul(&expaux[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[128]].signalStart + ((1 * ((8 * Fr_toInt(&lvar[2])) + Fr_toInt(&lvar[4]))) + 0)],&expaux[2]); // line circom 47
...
Keccak_256_bytes_5_create(mySignalStart+192,0+ctx_index+1,ctx,new_cmp_name,myId);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[66]); // line circom 14
while(Fr_isTrue(&expaux[0])){
{
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 0)],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 160)]); // line circom 15
}}
